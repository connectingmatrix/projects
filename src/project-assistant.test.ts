...
test('project assistant mounts filtered source, queries DB, logs debug, builds, deploys and heartbeats', async () => {
  ...
  const monitor = {
    start(input: { title: string }) { const id = `proc_${processEvents.length}`; processEvents.push(`${id}:start:${input.title}`); return { id }; },
    heartbeat(id: string, input?: { status?: string; message?: string }) { processEvents.push(`${id}:heartbeat:${input?.status ?? 'healthy'}:${input?.message ?? ''}`); },
    appendLog(id: string, level: string, message: string) { processEvents.push(`${id}:log:${level}:${message}`); },
    complete(id: string) { processEvents.push(`${id}:complete`); },
    fail(id: string, error: unknown) { processEvents.push(`${id}:fail:${String(error)}`); },
    abort(id: string, reason?: string) { processEvents.push(`${id}:abort:${reason ?? ''}`); },
    logs: { list: (id: string) => processEvents.filter((event) => event.startsWith(`${id}:`)) },
  };
  ...
  assert.equal((out.queryResults?.[0] as { databaseId?: string }).databaseId, db.id);
